...
class NavitiaScraper:
    ...
    def __init__(self, *, api_key: Optional[str] = None, mode: Optional[str] = None) -> None:
        configured_mode = (mode or settings.navitia_scraper_mode or "live").strip().lower()
        if configured_mode in {"disabled", "off"}:
            raise RuntimeError("Navitia scraper disabled by configuration")
        if configured_mode not in {"", "live"}:
            raise RuntimeError(
                f"Unsupported NAVITIA_SCRAPER_MODE '{configured_mode}'. Remove mock modes and use live data."
            )

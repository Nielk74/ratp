...
export default function OrchestratorDashboard() {
  ...
  const handleCommand = useCallback(async (workerId: string, command: string) => {
    try {
  ...
  const handleSchedulerTrigger = useCallback(async () => {
    try {
      setCommandState({ loading: true, message: null, error: null });
  ...
  const pendingTasks = useMemo(() => tasks.filter((task) => [\"queued\", \"pending\", \"running\"].includes(task.status)).length, [tasks]);
          ...
          <div className=\"grid grid-cols-1 sm:grid-cols-3 gap-4\">
            ...
            <div className=\"p-4 border rounded-lg\">
              ...
              <p className=\"text-base text-gray-900\">
                {queue?.last_scheduled_at
                  ? new Date(queue.last_scheduled_at).toLocaleTimeString()
                  : \"Not yet available\"}
              </p>
            </div>
          </div>
              ...
              <tbody className=\"divide-y divide-gray-100\">
                {tasks.map((task) => (
                  <tr key={task.job_id}>
                    ...
                    <td className=\"px-4 py-2 text-gray-700\">{task.network} {task.line}</td>

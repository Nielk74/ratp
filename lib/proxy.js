...
async function testAll(candidates) {
  return new Promise((resolve) => {
    ...
    function emitIfEnough() {
      if (!resolved && working.length >= QUICK_POOL_SIZE) {
        resolved = true;
        console.log(`[proxy] ${working.length} proxies found — returning immediately`);
        resolve(working.slice().sort((a, b) => a.latency - b.latency));
      }
    }

    function scheduleNext() {
      while (active < BATCH_SIZE && idx < candidates.length) {
        const candidate = candidates[idx++];
        active++;
        testProxy(candidate).then((result) => {
          active--;
          if (result) {
            working.push(result);
            if (!resolved) {
              console.log(`[proxy] Found: ${result.url} (${result.latency}ms) — ${working.length}/${QUICK_POOL_SIZE}`);
            }
            emitIfEnough();
          }
          if (!resolved) scheduleNext();
        });
      }
      if (idx >= candidates.length && active === 0 && !resolved) {
        resolved = true;
        resolve(working.sort((a, b) => a.latency - b.latency));
      }
    }
...
async function refresh() {
  ...
  try {
    ...
    console.log(`[proxy] Testing ${candidates.length} candidates (${BATCH_SIZE} concurrent, ${CONNECT_TIMEOUT}ms timeout, need ${QUICK_POOL_SIZE} to start)...`);
